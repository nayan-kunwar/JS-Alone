...
const promiseThree = new Promise(function(resolve, reject){
    setTimeout(function(){
        console.log("Async task 1 is completed.");
        resolve({name: "aman", age: 22}); // data will be passed to .then callback fn
    }, 1000);
});
...
async function consumePromiseFive(){ // await keyword is used with that line which takes some time to complete.
     try{
        //promiseFive is object so dont consume it like promiseFive();
        const response = await promiseFive;  // If error is false then response will contain the resolve data -> {name: "aman", age: 22}
        console.log(response);
    }
    catch(error){ // If we dont use try catch then , when error is in the code then we will not receive the reject data insted we will get not readable error
        console.log(error); // If error is true(error in the code) then this will catch the reject data -> ERROR: Something went wrong.
    }
}
consumePromiseFive();
